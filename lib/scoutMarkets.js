...
function estimateCashFlow(metro, price, rent, stateCode) {
  ...
  const taxMo      = price * ((STATE_TAX[stateCode] || 1.0) / 100) / 12;
  const insMo      = price * ((STATE_INS[stateCode] || 1.0) / 100) / 12;
  const vacMo      = rent * 0.08;
  const mgmtMo     = (rent - vacMo) * 0.10;
  const maintMo    = price * 0.01 / 12;
  const capexMo    = 150; // baseline CapEx
  const total      = mortgage + taxMo + insMo + vacMo + mgmtMo + maintMo + capexMo;
  return Math.round(rent - total);
}
