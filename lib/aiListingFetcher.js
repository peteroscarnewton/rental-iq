...
function buildPrompt(url) {
  return `Find this property listing and extract all available data: ${url}

Search Google for this listing. Look for: price, beds, baths, square footage, year built, annual property taxes, monthly HOA fee, and location.

Return this exact JSON (fill every field you can find, null for anything not found):
{
  "price": <number: list/purchase price in dollars, null if rental-only>,
  "rent": <number: monthly rent in dollars, null if for-sale>,
  "beds": <number: bedroom count>,
  "baths": <number: total baths, half-baths count as 0.5>,
  "sqft": <number: interior living area in sq ft>,
  "year": <number: year originally built>,
  "city": <string: "City, ST">,
  "taxAnnual": <number: annual property tax in dollars>,
  "hoaMonthly": <number: monthly HOA fee in dollars, 0 if no HOA>
}`;
}
