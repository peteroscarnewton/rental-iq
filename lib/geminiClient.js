...
// Ranked list: most preferred first, oldest-but-stable last.
// gemini-2.5-flash is GA and the current default per Google's API reference.
...
export async function callGemini(apiKey, payload, opts = {}) {
  const { timeoutMs = 50000, retries = 2 } = opts;
  const errors = [];
