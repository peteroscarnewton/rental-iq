 ...
 *   https://data.insideairbnb.com/united-states/{state}/{city}/
 * which lists all available snapshot dates as links.
 *
 * Falls back to KNOWN_DATE_SLUGS if discovery fails or times out.
 *
 * @param {string} baseUrl - e.g. "https://data.insideairbnb.com/united-states"
...
export async function fetchStrData(city, beds = 2) {
  ...
  try {
    ...
    //   https://data.insideairbnb.com/{country}/{state}/{city}/{date}/visualisations/listings.csv
    // The listings.csv (visualisations version) is ~500KB and has the fields we need.
    const baseUrl = `https://data.insideairbnb.com/united-states`;
