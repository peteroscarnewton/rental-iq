...
export function RentScenarios({data}) {
  ...
  return (
    <Card>
      ...
      <div className="riq-g3" style={{display:'grid',gridTemplateColumns:'repeat(3,minmax(0,1fr))',gap:8}}>
        {data.rentScenarios.map((s,i)=>{
          ...
          return (
            <div key={i} style={{background:isBase?C.soft:C.white,border:`${isBase?'2px':'1px'} solid ${isBase?C.green:C.border}`,borderRadius:14,padding:'14px 8px',textAlign:'center',position:'relative',minWidth:0,overflow:'hidden'}}>
              ...
              <div style={{fontSize:11.5,color:C.muted,marginBottom:4,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{s.rent}</div>
              <div style={{fontFamily:"'Instrument Serif',Georgia,serif",fontSize:19,lineHeight:1,marginBottom:4,color:isNeg?C.red:isPos?C.green:C.amber,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>{s.cashflow}</div>
              <div style={{fontSize:10.5,color:C.muted,marginBottom:7}}>CoC: {s.coc}</div>
